...
GraphEmitter.prototype.destroy = function(){ console.log('GraphEmitter.destroy')
  this.stream.destroy();
};

GraphEmitter.prototype.setQueryEmitter = function(queryEmitter){ console.log('GraphEmitter.setQueryEmitter')
  this.queryEmitter = queryEmitter;
};

GraphEmitter.prototype.updateQueryEmitterQuery = function(query){ console.log('GraphEmitter.updateQueryEmitterQuery')
  if (this.queryEmitter) {
    this.queryEmitter.updateQuery(query);
  }
};

GraphEmitter.prototype.startStream = function(){ console.log('GraphEmitter.startStream')
  var emitter = this;
  function readStream() {
    var data;
...
GraphEmitter.prototype._emitTiming = function(action, start){ console.log('GraphEmitter._emitTiming')
  this.backend.emit('timing', action, Date.now() - start, this.index, this.query);
};

GraphEmitter.prototype.update = function(op){ console.log('GraphEmitter.update')
  var id = op.d;
...
GraphEmitter.prototype._flushPoll = function(){ console.log('GraphEmitter._flushPoll')
  if (this._polling || this._pollTimeout) return;
  if (this._pollAgain) this.queryPoll();
};

GraphEmitter.prototype._finishPoll = function(err){ console.log('GraphEmitter._finishPoll')
  this._polling = false;
  if (err) this.emitError(err);
  this._flushPoll();
};

GraphEmitter.prototype.queryPoll = function(){ console.log('GraphEmitter.queryPoll')
  var emitter = this,
      self = this;
...
GraphEmitter.prototype.emitError = function(err){ console.log('GraphEmitter.emitError')
  this.onError(err);
};

GraphEmitter.prototype.emitDiff = function(diff){ console.log('GraphEmitter.emitDiff')
};

GraphEmitter.prototype.emitExtra = function(extra){ console.log('GraphEmitter.emitExtra')
};

GraphEmitter.prototype.emitOp = function(op){ console.log('GraphEmitter.emitOp')
};
...
GraphEmitter.prototype.onOp = function() { console.log('GraphEmitter.onOp')
