...
Query.prototype.hasPending = function() { console.log('Query.hasPending')
  return !this.ready;
};

Query.prototype.setQuery = function(query) { console.log('Query.setQuery')
  this.query = query;
};
...
Query.prototype.destroy = function(callback) { console.log('Query.destroy')
...
Query.prototype._onConnectionStateChanged = function() { console.log('Query._onConnectionStateChanged')
...
Query.prototype._handleFetch = function(err, data, extra) { console.log('Query._handleFetch')
  // Once a fetch query gets its data, it is destroyed.
  this.connection._destroyQuery(this);
  this._handleResponse(err, data, extra);
};

Query.prototype._handleSubscribe = function(err, data, extra) { console.log('Query._handleSubscribe')
  this._handleResponse(err, data, extra);
};

Query.prototype._handleResponse = function(err, data, extra) { console.log('Query._handleResponse')
...
Query.prototype._digestSnapshots = function(snapshots) { console.log('Query._digestSnapshots')
...
Query.prototype._ingestSnapshots = function(snapshots, finish) { console.log('Query._ingestSnapshots')
  var results = [];
  for (var i = 0; i < snapshots.length; i++) {
...
Query.prototype._finishResponse = function(err, callback) { console.log('Query._finishResponse')
  this.emit('ready');
  this.ready = true;
  if (err) {
    this.connection._destroyQuery(this);
...
Query.prototype._handleError = function(err) { console.log('Query._handleError')
  this.emit('error', err);
};

Query.prototype._handleDiff = function(diff) { console.log('Query._handleDiff')
...
Query.prototype._handleExtra = function(extra) { console.log('Query._handleExtra')
  this.extra = extra;
