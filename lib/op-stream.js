...
OpStream.prototype.initProjection = function(backend, agent, projection) { console.log('OpStream.initProjection')
  this.backend = backend;
  this.agent = agent;
  this.projection = projection;
};

OpStream.prototype.pushOp = function(collection, id, op) { console.log('OpStream.pushOp')
  if (this.backend) {
    var stream = this;
...
OpStream.prototype.pushOps = function(collection, id, ops) { console.log('OpStream.pushOps')
  for (var i = 0; i < ops.length; i++) {
    this.pushOp(collection, id, ops[i]);
  }
};

OpStream.prototype.destroy = function() { console.log('OpStream.destroy')
  if (!this.open) return;
  this.open = false;
