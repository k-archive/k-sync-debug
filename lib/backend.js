...
Backend.prototype.close = function(callback) { console.log('Backend.close')
  var wait = 3;
  var backend = this;
  function finish(err) {
    if (err) {
...
Backend.prototype.connect = function(connection, req) { console.log('Backend.connect')
 ...
 */
Backend.prototype.listen = function(stream, req) { console.log('Backend.listen')
...
Backend.prototype.addRPC = function(name, fn) { console.log('Backend.addRPC')
  this.rpc[name] = fn;
}

Backend.prototype.addFunction = function(name, code) { console.log('Backend.addFunction')
  this.functions[name] = code;
}

Backend.prototype.addProjection = function(name, collection, fields) { console.log('Backend.addProjection')
 ...
 */
Backend.prototype.use = function(action, fn) { console.log('Backend.use')
 ...
 */
Backend.prototype.trigger = function(action, agent, request, callback) { console.log('Backend.trigger')
...
Backend.prototype.submit = function(agent, index, id, op, callback) { console.log('Backend.submit')
...
Backend.prototype._sanitizeOp = function(agent, projection, collection, id, op, callback) { console.log('Backend._sanitizeOp')
  if (projection) {
    try {
...
Backend.prototype._sanitizeOps = function(agent, projection, collection, id, ops, callback) { console.log('Backend._sanitizeOps')
  var backend = this;
  async.each(ops, function(op, eachCb) {
    backend._sanitizeOp(agent, projection, collection, id, op, eachCb);
  }, callback);
};
Backend.prototype._sanitizeOpsBulk = function(agent, projection, collection, opsMap, callback) { console.log('Backend._sanitizeOpsBulk')
  var backend = this;
...
Backend.prototype._sanitizeSnapshot = function(agent, projection, collection, id, snapshot, callback) { console.log('Backend._sanitizeSnapshot')
  if (projection) {
    try {
...
Backend.prototype._sanitizeSnapshots = function(agent, projection, collection, snapshots, callback) { console.log('Backend._sanitizeSnapshots')
  var backend = this;
...
Backend.prototype._sanitizeSnapshotBulk = function(agent, projection, collection, snapshotMap, callback) { console.log('Backend._sanitizeSnapshotBulk')
  var backend = this;
...
Backend.prototype._getSnapshotProjection = function(db, projection) { console.log('Backend._getSnapshotProjection')
...
Backend.prototype.getOps = function(agent, index, id, from, to, callback) { console.log('Backend.getOps')
  var start = Date.now();
...
Backend.prototype.getOpsBulk = function(agent, index, fromMap, toMap, callback) { console.log('Backend.getOpsBulk')
  var start = Date.now();
  var projection = this.projections[index];
  var collection = (projection) ? projection.target : index;
  var backend = this;
...
Backend.prototype.fetch = function(agent, index, id, callback) { console.log('Backend.fetch')
  var start = Date.now();
  var projection = this.projections[index];
  var collection = (projection) ? projection.target : index;
...
Backend.prototype.subscribe = function(agent, index, id, version, callback) { console.log('Backend.subscribe')
  var start = Date.now();
  var projection = this.projections[index];
  var collection = (projection) ? projection.target : index;
...
Backend.prototype.subscribeBulk = function(agent, index, versions, callback) { console.log('Backend.subscribeBulk')
  var start = Date.now();
  var projection = this.projections[index];
  var collection = (projection) ? projection.target : index;
  var backend = this;
...
Backend.prototype.queryFetch = function(agent, index, query, options, callback) { console.log('Backend.queryFetch')
  var start = Date.now();
  var backend = this;
...
Backend.prototype.functionFetch = function(agent, index, query, options, callback) { console.log('Backend.functionFetch')
  var start = Date.now();
  var backend = this;
...
//  liveness
Backend.prototype.querySubscribe = function(agent, index, query, options, callback) { console.log('Backend.querySubscribe')
  var start = Date.now();
  var backend = this;
  backend._triggerQuery(agent, index, query, options, function(err, request) {
    if (err) return callback(err);
...
Backend.prototype._triggerFunctionFetch = function(agent, index, query, options, callback) { console.log('Backend._triggerFunctionFetch')
  var request = {
    fnname: index,
    query: query,
    options: options,
...
Backend.prototype._triggerQuery = function(agent, index, query, options, callback) { console.log('Backend._triggerQuery')
  var projection = this.projections[index];
  var collection = (projection) ? projection.target : index;
  var fields = projection && projection.fields;
  var request = {
...
Backend.prototype._query = function(agent, request, callback) { console.log('Backend._query')
  var backend = this;
...
Backend.prototype.getCollectionChannel = function(collection) { console.log('Backend.getCollectionChannel')
  return collection;
};

Backend.prototype.getDocChannel = function(collection, id) { console.log('Backend.getDocChannel')
  return collection + '.' + id;
};

Backend.prototype.getChannels = function(collection, id) { console.log('Backend.getChannels')
  return [
